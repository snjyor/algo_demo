...
def get_data():
    f = open("data/sanguo.txt", 'r',encoding='utf-8') #读入文本
    lines = []
    for line in f: #分别对每段分词
        temp = jieba.cut(line)  #结巴分词 精确模式
        words = []
        for i in temp:
            #过滤掉所有的标点符号
            i = re.sub("[\s+\.\!\/_,$%^*(+\"\'””《》]+|[+——！，。？、~@#￥%……&*（）：；‘]+", "", i)
...
def vision(model):
    ...
    return word2ind, rawWordVec, X_reduced


def plot_pic(word2ind, X_reduced):
    # 绘制星空图
    # 绘制所有单词向量的二维空间投影
...
def main():
    lines = get_data()
    model = wv_train(lines)
    word2ind, rawWordVec, X_reduced = vision(model)
    plot_pic(word2ind, X_reduced)
    similar_text(model)
